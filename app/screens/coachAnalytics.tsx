    ...
    <SafeAreaView style={styles.screen} edges={["top", "bottom"]}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <AnalyticsCard title={coach.headline} subtitle="Current interpretation">
          <View style={[styles.statusBar, { borderLeftColor: toneColor(coach.tone, palette) }]}>
        ...
        <AnalyticsCard title="What to do next" subtitle="Action based on the combined evidence">
          <View style={styles.actionRow}>
            <MaterialIcons name="arrow-forward" size={22} color={palette.primary} />
          ...
          >
            <AnalyticsCard title={item.label} subtitle={sourceLabel(item.source)}>
              <View style={styles.evidenceHeader}>
                <Text style={[styles.evidenceValue, { color: toneColor(item.tone, palette) }]}>
                  {item.value}
                </Text>
                {canOpen(item.source) ? (
...
const createStyles = (palette: ReturnType<typeof useAppPalette>) => StyleSheet.create({
  ...
  title: { color: palette.text, fontSize: 29, fontWeight: "900", marginTop: 3 },
  subtitle: { color: palette.textMuted, fontSize: 13, lineHeight: 19, marginTop: 6, marginBottom: 14 },
  sectionLabel: { color: palette.primary, fontSize: 12, fontWeight: "800", letterSpacing: 1.2, marginTop: 22, marginBottom: 8 },
  statusBar: { borderLeftWidth: 4, paddingLeft: 12, marginTop: 8 },
  summary: { color: palette.text, fontSize: 14, lineHeight: 21 },
  actionRow: { flexDirection: "row", alignItems: "flex-start", gap: 10, marginTop: 8 },
  actionText: { flex: 1, color: palette.text, fontSize: 14, lineHeight: 21, fontWeight: "600" },
  ...
  evidenceText: { color: palette.textMuted, fontSize: 13, lineHeight: 19, marginTop: 7 },
  pressed: { opacity: 0.72 },
  disclaimer: { color: palette.textMuted, fontSize: 11, lineHeight: 16, marginTop: 18, textAlign: "center" },
});
